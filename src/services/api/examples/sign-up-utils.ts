 ...
 */
export function parseSignUpResponse(response: BaseResponseModel<any>): SignUpParseResult {
...
export function mapBackendFieldsToFormFields(
  ...
  Object.entries(fieldErrors).forEach(([key, value]) => {
    ...
    const formField = key === 'first_name' ? 'firstName' : 
 ...
 */
export function getSignUpErrorMessage(response: BaseResponseModel<any>): string {
  if (!isErrorResponse(response)) {
    return "";
  }
  ...
  return errorTranslations[backendMessage] || backendMessage || "Registration failed. Please try again.";
}
...
export function logSignUpAttempt(
  ...
  if (isSuccessResponse(response)) {
    ...
    console.warn(`⚠️ [${timestamp}] Unknown sign-up response for: ${email}`, response);
  }
}
